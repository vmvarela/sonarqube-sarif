...
The action does five things in order:

1. Parse and validate GitHub Action inputs.
2. Optionally wait for SonarQube processing to finish.
3. Fetch issues, components, and rule metadata from SonarQube.
4. Filter the result set by severity and, in PRs, by changed files.
5. Convert the remaining issues to SARIF, write the file, and publish feedback back into GitHub.
...
```mermaid
flowchart TD
    A[Parse inputs] --> B{Wait for processing?}
    B -->|yes| C[Poll /api/ce/component]
    B -->|no| D[Optional fixed delay]
    C --> E[Fetch /api/issues/search]
    D --> E
    E --> F[Fetch missing rule details]
    F --> G[Filter by severity]
    G --> H{Pull request?}
    H -->|yes| I[Fetch changed PR files]
    H -->|no| J[Convert to SARIF]
    I --> K[Keep issues in changed files]
    K --> J
    J --> L[Write SARIF file]
    L --> M[Set outputs and job summary]
    M --> N[Create Check Run]
    N --> O[Optional PR comment]
```
...
1. **Bad configuration** — invalid URL, bad integers, missing token.
2. **SonarQube auth or permission problems** — usually `401` or `403`.
3. **Connectivity or timeout issues** — bad host, server unavailable, or slow responses.
4. **Partial GitHub integration issues** — PR comment or Check Run creation fails, but SARIF generation still succeeds when possible.

This split is deliberate. Fetching from SonarQube is core behavior; PR comments are optional sugar.
